...
export default async function DocumentPreviewPage({params}:{params:Promise<{type:string;id:string}>}){
 ...
 if(type==="contract"){
  const [{data:organization,error:organizationError},{data:contract,error:contractError}]=await Promise.all([
   ...
   supabase.from("crm_contracts").select("id,contract_no,title,scope,amount,currency,payment_plan,payment_plan_type,payment_schedule,start_date,due_date,status,created_at,signed_name,signed_at,signed_signature_data,signed_ip,contract_template_key,contract_template_version,crm_opportunities(customer_name,contact_email,contact_phone),crm_proposals(payment_schedule)").eq("id",id).eq("organization_id",membership.organization_id).maybeSingle(),
  ]);
