...
export default async function PaymentPlansPage() {
  ...
  return <div className="finance-page-stack">
    <div className="panel-pagehead">
      <div><small className="panel-kicker">FİNANS / TAHSİLAT</small><h1>Ödeme Planları</h1><p>İmzalı sözleşmelerden otomatik oluşan taksitleri takip edin ve tahsilatları cari hesaba işleyin.</p></div>
    ...
    <section className="finance-plan-list">
      {planRows.map((plan) => {
        ...
        const paidAmount = items.filter((item) => item.status === "paid").reduce((sum, item) => sum + Number(item.amount), 0);
          ...
          <div className="finance-installments">
            {items.map((item) => {
              ...
              return <div className={isOverdue ? "finance-installment overdue" : "finance-installment"} key={item.id}>
                ...
                {item.status === "pending" ? <form action={collectPaymentInstallment}><input type="hidden" name="installment_id" value={item.id} /><button className="panel-primary" type="submit">Tahsil Et</button></form> : null}
              </div>;
            })}
