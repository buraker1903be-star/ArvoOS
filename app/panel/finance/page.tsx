...
export default async function FinancePage({
  ...
  const [{ data, error }, { data: contractData, error: contractError }, { data: installmentData, error: installmentError }, {data:costItemData,error:costItemError},{data:employeeData},{data:workflowData}] =
    await Promise.all([
      ...
      supabase
        .from("crm_contracts")
        .select("id,party_id,amount,status,contract_no,title,currency,payment_plan_id,workflow_id,service_cost,service_cost_supplier,service_cost_reference,service_cost_status,crm_opportunities(customer_name,contact_phone,contact_email,assigned_employee_id)")
        .eq("organization_id", membership.organization_id)
  ...
  return (
    <main className="finance-ledger-page">
      ...
      {mode==="maliyet"&&canManageCosts ? <section className="finance-cost-workspace"><div className="finance-cost-kpis"><article><small>TOPLAM SÖZLEŞME</small><strong>{money(contracts.reduce((s,c)=>s+Number(c.amount),0))}</strong><span>{contracts.length} iş</span></article><article><small>MALİYET TOPLAMI</small><strong>{money(totalServiceCost)}</strong><span>İşlere bağlı giderler</span></article><article className="is-profit"><small>TOPLAM KÂR</small><strong>{money(contracts.reduce((s,c)=>s+Number(c.amount)-(costTotals.get(c.id)??Number(c.service_cost)),0))}</strong><span>Brüt iş kârlılığı</span></article><article><small>ORTALAMA KÂR ORANI</small><strong>%{Math.round(contracts.reduce((s,c)=>s+(Number(c.amount)?((Number(c.amount)-(costTotals.get(c.id)??Number(c.service_cost)))/Number(c.amount))*100:0),0)/(contracts.length||1))}</strong><span>Sözleşme bazında</span></article></div><section className="panel-card finance-contract-costs">
        <header><div><small className="panel-kicker">YÖNETİCİ FİNANSI</small><h2>İş Kârlılık Tablosu</h2><p>Sözleşmeye tıklayarak ayrıntılı maliyet hareketlerini açın.</p></div></header>
        <div className="finance-cost-table"><div className="finance-cost-table-head"><span>İş / Müşteri</span><span>Satış</span><span>Operasyon</span><span>Sözleşme</span><span>Maliyet</span><span>Toplam Kâr</span><span>Kâr Oranı</span><span></span></div>{contracts.map(contract=>{const relation=Array.isArray(contract.crm_opportunities)?contract.crm_opportunities[0]:contract.crm_opportunities;const cost=costTotals.get(contract.id)??Number(contract.service_cost);const profit=Number(contract.amount)-cost;const margin=Number(contract.amount)?(profit/Number(contract.amount))*100:0;const operationEmployeeId=contract.workflow_id?workflowMap.get(contract.workflow_id):null;return <article key={contract.id}><Link href={`/panel/finance/costs/${contract.id}`}><span>{contract.contract_no}</span><h3>{relation?.customer_name||contract.title}</h3><p>{contract.title}</p></Link><span className="finance-person-cell">{relation?.assigned_employee_id?employeeMap.get(relation.assigned_employee_id)||"Pasif personel":"Atanmamış"}</span><span className="finance-person-cell">{operationEmployeeId?employeeMap.get(operationEmployeeId)||"Pasif personel":"Atanmamış"}</span><strong>{money(contract.amount)}</strong><strong className="is-cost">{money(cost)}</strong><strong className={profit>=0?"is-positive":"is-negative"}>{money(profit)}</strong><span className={`finance-margin ${margin>=30?"good":margin>=0?"medium":"bad"}`}>%{margin.toFixed(1)}</span><Link className="panel-secondary" href={`/panel/finance/costs/${contract.id}`}>Detay</Link></article>})}{!contracts.length?<p className="panel-empty">İmzalanmış sözleşme bulunmuyor.</p>:null}</div>
