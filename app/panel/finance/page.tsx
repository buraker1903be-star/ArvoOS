...
function daysOverdue(date:string|null) {
  if (!date) return 0;
  const due = new Date(date.includes("T") ? date : `${date}T00:00:00`);
...
export default async function FinancePage() {
  ...
  const invoiceItems = (invoices ?? []) as Invoice[];
  const accountList = (bankAccounts ?? []) as BankAccount[];
  const bankMoves = (bankTransactions ?? []) as BankTransaction[];
  const ledger = (accountEntries ?? []) as AccountEntry[];
  ...
  const paidIncome = items.filter((item) => item.transaction_type === "income" && item.status === "paid").reduce((sum,item) => sum + Number(item.amount),0)
    + invoiceItems.filter((item) => item.status === "paid").reduce((sum,item) => sum + Number(item.total ?? 0),0);
  ...
  const expectedIncome = items.filter((item) => item.transaction_type === "income" && item.status === "planned").reduce((sum,item) => sum + Number(item.amount),0)
    + invoiceItems.filter((item) => item.status === "open").reduce((sum,item) => sum + Number(item.total ?? 0),0);
  ...
  const bankOpening = accountList.reduce((sum,item) => sum + Number(item.opening_balance ?? 0),0);
  const bankInflows = bankMoves.filter((item) => item.direction === "inflow").reduce((sum,item) => sum + Number(item.amount),0);
  const bankOutflows = bankMoves.filter((item) => item.direction === "outflow").reduce((sum,item) => sum + Number(item.amount),0);
  const bankBalance = bankOpening + bankInflows - bankOutflows;
  const unmatchedCount = bankMoves.filter((item) => item.reconciliation_status === "unmatched").length;

  const receivables = ledger.filter((item) => item.entry_type === "debit");
  const aging = {
    current: 0,
    d30: 0,
    d60: 0,
  ...
  for (const entry of receivables) {
    const overdue = daysOverdue(entry.due_date);
    const amount = Number(entry.amount);
    if (!entry.due_date || overdue === 0) aging.current += amount;
    else if (overdue <= 30) aging.d30 += amount;
    else if (overdue <= 60) aging.d60 += amount;
    else if (overdue <= 90) aging.d90 += amount;
    else aging.over90 += amount;
  }

  const upcoming = [
    ...items.filter((item) => item.status === "planned" && item.due_date).map((item) => ({ id:`tx-${item.id}`, label:item.title, amount:item.amount, date:item.due_date!, type:item.transaction_type })),
    ...invoiceItems.filter((item) => item.status === "open" && item.due_at).map((item) => ({ id:`inv-${item.id}`, label:"Açık fatura", amount:item.total, date:item.due_at!, type:"income" as const })),
  ].sort((a,b) => new Date(a.date).getTime() - new Date(b.date).getTime()).slice(0,8);

  const canManage = ["owner","admin"].includes(membership.role);

  return <>
    <div className="panel-pagehead"><div><small className="panel-kicker">FİNANS MERKEZİ 2.0</small><h1>Nakit, banka ve cari riskini tek ekranda yönetin</h1><p>Gerçekleşen akış, açık tahsilatlar, banka mutabakatı ve cari yaşlandırmayı kurum bazında izleyin.</p></div><span className="status-pill">{items.length + invoiceItems.length} finans kaydı</span></div>

    <section className="finance-metrics finance-metrics-six">
      ...
      <article><small>EŞLEŞMEYEN BANKA</small><strong>{unmatchedCount}</strong><span>Mutabakat bekleyen hareket</span></article>
    </section>

    <section className="finance-insights">
      <article className="panel-card aging-card">
        <div><small>CARİ YAŞLANDIRMA</small><h3>Alacakların vade dağılımı</h3></div>
        <div className="aging-grid">
          <span><b>{money(aging.current)}</b><small>Vadesi gelmedi</small></span>
          <span><b>{money(aging.d30)}</b><small>1–30 gün</small></span>
          <span><b>{money(aging.d60)}</b><small>31–60 gün</small></span>
          <span><b>{money(aging.d90)}</b><small>61–90 gün</small></span>
          <span className="risk"><b>{money(aging.over90)}</b><small>90+ gün</small></span>
        </div>
      </article>
      <article className="panel-card due-card">
        <div><small>YAKLAŞAN VADELER</small><h3>Öncelikli nakit hareketleri</h3></div>
        {upcoming.map((item) => <div className="due-row" key={item.id}><div><b>{item.label}</b><small>{new Date(item.date).toLocaleDateString("tr-TR")}</small></div><strong className={item.type === "expense" ? "negative" : "positive"}>{item.type === "expense" ? "-" : "+"}{money(Number(item.amount))}</strong></div>)}
        {!upcoming.length ? <p className="finance-empty">Yaklaşan vadeli kayıt bulunmuyor.</p> : null}
      </article>
    </section>
    ...
    <section className="finance-grid">
      ...
      <aside className="panel-card finance-side"><small>BANKA HESAPLARI</small><h3>Aktif kurum hesapları</h3>{accountList.map((account) => { const moves = bankMoves.filter((move) => move.bank_account_id === account.id); const balance = Number(account.opening_balance ?? 0) + moves.filter((move) => move.direction === "inflow").reduce((sum,move) => sum + Number(move.amount),0) - moves.filter((move) => move.direction === "outflow").reduce((sum,move) => sum + Number(move.amount),0); return <div className="bank-card" key={account.id}><b>{account.bank_name}</b><span>{account.account_name || "Hesap sahibi belirtilmedi"}</span><code>{String(account.iban).replace(/(.{4})/g,"$1 ").trim()}</code><strong>{money(balance)}</strong></div>})}<hr /><small>AÇIK FATURALAR</small><strong>{invoiceItems.filter((item) => item.status === "open").length}</strong><p>CRM ve abonelik akışından oluşan tahsilat kayıtları.</p></aside>
    </section>
  </>;
}
