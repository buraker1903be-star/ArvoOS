...
export default async function PlatformPage({ searchParams }: { searchParams: Promise<{ organization?: string }> }) {
  ...
  const { data: organizationData, error: organizationError } = await supabase.from("organizations").select("id,name,slug,status,plan_code,sector,custom_domain").order("name");
  ...
  return <>
    ...
    <section className="platform-overview"><div><small>CANLI KURUM ÖZETİ</small><h2>{selectedOrganization.name}</h2><p>Seçilen kurumun paketi, modülleri ve kullanım özeti güvenli kurucu politikaları üzerinden yönetilir.</p></div><dl><div><dt>KURUM</dt><dd>{selectedOrganization.name}</dd></div><div><dt>PAKET</dt><dd>{selectedOrganization.plan_code}</dd></div><div><dt>AKTİF MODÜL</dt><dd>{enabledCount}</dd></div><div><dt>KULLANICI</dt><dd>{memberCount ?? 0}</dd></div></dl></section>
