...
const defaultColumns: Stage[] = [
  { code: "lead", name: "Yeni", probability: 10, is_terminal: false },
  { code: "qualified", name: "İnceleniyor", probability: 25, is_terminal: false },
  { code: "proposal", name: "Teklif Hazırlanıyor", probability: 50, is_terminal: false },
  { code: "contract", name: "Sözleşme", probability: 70, is_terminal: false },
  { code: "payment", name: "Tahsilat", probability: 90, is_terminal: false },
  { code: "won", name: "Kabul Edildi", probability: 100, is_terminal: true },
...
export default async function RequestsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  if (stageError) throw new Error("Talep durumları okunamadı: " + stageError.message);

  const stages = ((configuredStages?.length ? configuredStages : defaultColumns) ?? []) as Stage[];
  const stageMap = new Map(stages.map((stage) => [stage.code, stage]));
  const academicMode = stages.some((stage) => stage.code === "academic_review");
  const terminalCodes = new Set(stages.filter((stage) => stage.is_terminal).map((stage) => stage.code));
  const allRows = (data ?? []) as Opportunity[];
  const rows = allRows.filter((item) => {
    const haystack = [item.customer_name, item.title, item.contact_email, item.contact_phone, item.request_details?.service_type]
      .filter(Boolean).join(" ").toLocaleLowerCase("tr-TR");
    const searchMatch = !search || haystack.includes(search);
    const stageMatch = selectedStage === "tumu" ? true : selectedStage ? item.stage === selectedStage : !terminalCodes.has(item.stage);
    return searchMatch && stageMatch;
  });
  const active = allRows.filter((item) => !terminalCodes.has(item.stage));
  const newRequests = allRows.filter((item) => item.stage === "lead");
  const archived = allRows.filter((item) => terminalCodes.has(item.stage));
  ...
  const requestForm = <form className="panel-form" action={createOpportunity}>
    {academicMode ? <><label>Müşteri türü<select name="customer_type" defaultValue="Bireysel"><option>Bireysel</option><option>Kurumsal</option></select></label><label>Hizmet türü<select name="service_type" required defaultValue=""><option value="" disabled>Seçin</option><option>Tez Danışmanlığı</option><option>Makale Hazırlama</option><option>İstatistiksel Analiz</option><option>Editörlük ve Dil Kontrolü</option><option>Literatür Taraması</option><option>Sunum Hazırlama</option><option>Diğer</option></select></label></> : null}
    <label>Talep konusu<input name="title" required minLength={2} maxLength={180} /></label><label>Müşteri / kurum<input name="customer_name" required minLength={2} maxLength={180} /></label>
    <label>E-posta<input name="contact_email" type="email" /></label><label>Telefon<input name="contact_phone" /></label>
    ...
    <label>Beklenen tutar<input name="estimated_value" type="number" min="0" step="0.01" defaultValue="0" /></label><label>Planlanan teslim tarihi<input name="expected_close_date" type="date" /></label>
    <label>Talep kaynağı<input name="source" placeholder="Web sitesi, telefon, referans..." /></label><label className="wide">Ek not<textarea name="notes" /></label>
  ...
  return <div className="crm-page-stack">
    ...
    <section className="panel-card crm-filter-card"><form action="/panel/crm" method="get" className="crm-filter-form"><label><span>Müşteri / talep ara</span><input type="search" name="arama" defaultValue={rawSearch ?? ""} placeholder="Müşteri, konu, telefon veya e-posta" /></label><label><span>Durum</span><select name="durum" defaultValue={selectedStage}><option value="">Aktif Talepler</option><option value="tumu">Tüm Kayıtlar</option>{stages.map((stage) => <option value={stage.code} key={stage.code}>{stage.name}</option>)}</select></label><div><button className="panel-primary" type="submit">Filtrele</button><a className="panel-secondary" href="/panel/crm">Temizle</a></div></form></section>

    <section className="crm-record-list">
      {rows.map((item) => { const details = item.request_details ?? {}; const stage = stageMap.get(item.stage); return <article className="panel-card crm-record" key={item.id}>
        ...
        <aside className="crm-record-side"><small>BEKLENEN DEĞER</small><strong>{money(item.estimated_value)}</strong>{item.expected_close_date ? <span>Teslim: {new Date(item.expected_close_date + "T00:00:00").toLocaleDateString("tr-TR")}</span> : <span>Teslim tarihi yok</span>}<form action={moveOpportunity}><input type="hidden" name="opportunity_id" value={item.id} /><label>Durum<select name="stage" defaultValue={item.stage}>{stages.map((option) => <option value={option.code} key={option.code}>{option.name}</option>)}</select></label><label>Kayıp nedeni<input name="lost_reason" placeholder="Vazgeçildiyse belirtin" /></label><button className="panel-secondary" type="submit">Güncelle</button></form></aside>
      </article>; })}
      {!rows.length ? <section className="panel-card crm-empty-state"><h2>Eşleşen talep bulunamadı</h2><p>Arama ifadesini veya durum filtresini değiştirin.</p></section> : null}
    </section>
  </div>;
}
