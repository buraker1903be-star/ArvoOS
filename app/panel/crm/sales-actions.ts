...
export async function createProposal(formData: FormData) {
  ...
  const paymentPlan = text(formData, "payment_plan", 500);
  ...
  const { data, error } = await supabase.rpc("create_crm_proposal", {
  ...
  if (error) throw new Error("Teklif oluşturulamadı: " + error.message);
...
export async function issueProposalLink(formData: FormData) {
  const { supabase } = await getPanelContext();
  const proposalId = text(formData, "proposal_id", 80);
  const { data, error } = await supabase.rpc("issue_crm_proposal_link", { target_proposal_id: proposalId });
...
export async function issueContractLink(formData: FormData) {
  const { supabase } = await getPanelContext();
  const contractId = text(formData, "contract_id", 80);
  const { data, error } = await supabase.rpc("issue_crm_contract_link", { target_contract_id: contractId });
