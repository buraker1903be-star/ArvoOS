...
type CustomRow = { id: string; label: string; percentage: number; due_date: string };
const customRowPresets = ["Peşin (Sözleşme Onayında)", "Ön Ödeme", "Ara Ödeme", "Son Ödeme (Teslim Öncesi)"];
...
export function ProposalBuilderForm({ opportunityId, customerName, title, scope }: Props) {
  ...
  const [customRows, setCustomRows] = useState<CustomRow[]>([
    { id: nextRowId(), label: customRowPresets[0], percentage: 100, due_date: today() },
  ]);
  ...
  const customPercentTotal = customRows.reduce((sum, row) => sum + (Number(row.percentage) || 0), 0);
  const customSchedule: PaymentScheduleItem[] = useMemo(() => customRows.map((row, index) => ({
    sequence: index + 1,
    label: row.label || `${index + 1}. Ödeme`,
    due_date: row.due_date || today(),
    amount: Math.round((calculation.gross * (Number(row.percentage) || 0)) / 100),
  ...
  function addCustomRow() {
    setCustomRows((rows) => [...rows, { id: nextRowId(), label: customRowPresets[Math.min(rows.length, customRowPresets.length - 1)] ?? `${rows.length + 1}. Ödeme`, percentage: 0, due_date: today() }]);
  }
  function removeCustomRow(id: string) {
    setCustomRows((rows) => (rows.length > 1 ? rows.filter((row) => row.id !== id) : rows));
  }
  ...
  return <div className="proposal-drawer-content">
    ...
    <form className="panel-form proposal-builder" action={createProposal}>
      ...
      <label className="wide">Ödeme planı<select value={plan} onChange={(event) => setPlan(event.target.value as PaymentPlanType)}>
        ...
        <option value="half">2. %50 Peşin (Sözleşme Onayında) + %50 Teslim Öncesi</option>
        <option value="third">3. 1/3 Peşin (Sözleşme Onayında) + Ara Ödeme + Son Ödeme (Teslim Öncesi)</option>
      ...
      {plan === "custom" ? (
        <section className="wide custom-plan-editor">
          <div className="custom-plan-head">
            ...
            <span className={customPercentTotal === 100 ? "custom-plan-total ok" : "custom-plan-total warn"}>Toplam %{customPercentTotal.toFixed(0)}</span>
          </div>
      ...
      <div className="wide panel-form-actions"><button className="panel-primary" type="submit" disabled={plan === "custom" && customPercentTotal !== 100}>Teklifi Oluştur</button></div>
    </form>
  </div>;
}
