    ...
    <div className="crm-request-detail-page">
          ...
          <div className="share-ready-body">
            ...
            <div className="panel-page-actions">
              {customer?.contact_email && messages ? (
                <ShareSendLink kind="proposal" token={data.share_token} className="panel-primary" href={`mailto:${encodeURIComponent(customer.contact_email)}?subject=${encodeURIComponent(messages.subject)}&body=${encodeURIComponent(messages.email)}`}>
                  ✉ E-posta ile gönder
                </ShareSendLink>
              ) : null}
      ...
      <section className="panel-card crm-request-detail-card">
        <div className="crm-request-detail-heading">
          <div><span className="status-pill" data-tone={statusTone(data.status)}>{labels[data.status] ?? data.status}</span><h2>{data.title}</h2></div>
