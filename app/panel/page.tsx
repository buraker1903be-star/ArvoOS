...
export default function PanelPage() {
  ...
  if (!membership) {
    return <main className="panel-setup-page"><section className="panel-setup-card">
      <img src="/arvoos-logo.png" alt="ArvoOS" /><small>ARVOOS KURULUM SİHİRBAZI</small><h1>İlk çalışma alanınızı oluşturun</h1>
  ...
  return <main className="panel-shell">
    <aside className="panel-sidebar">
      ...
      <div className="panel-company"><small>AKTİF ÇALIŞMA ALANI</small>
        {memberships.length > 1 ? <select aria-label="Aktif çalışma alanı" value={membership.organization_id} onChange={(event) => void handleOrganizationChange(event.target.value)}>{memberships.map((item) => <option key={item.organization_id} value={item.organization_id}>{item.organization.name}</option>)}</select> : <b>{organization.name}</b>}
        <span>{planLabels[organization.plan]}</span>
      </div>
      <nav><button className="active">Genel Bakış</button>{visibleModules.map((module) => <button key={module.name} type="button" onClick={() => openModule(module.href)}>{module.name}</button>)}</nav>
      <button className="logout" type="button" onClick={handleLogout}>Çıkış Yap</button>
    </aside>

    <section className="panel-content">
      <header className="panel-header"><div><small>{organization.name.toUpperCase()} OPERASYON MERKEZİ</small><h1>Hoş geldiniz</h1><p>{session?.user.email} · {roleName}</p></div><span>{session?.user.email?.slice(0, 2).toUpperCase()}</span></header>
      {error && <div className="panel-error panel-error-wide" role="alert">{error}</div>}
      <section className="hero-card"><div><small>SİSTEM DURUMU</small><h2>{organization.name} çalışma alanı aktif.</h2><p>Menüler ve modüller, Supabase rol ve izin kayıtlarına göre gösteriliyor. Kullanıcı yalnızca yetkili olduğu çalışma alanlarını ve modülleri görebilir.</p></div><strong>{organization.is_active ? "Aktif" : "Askıda"}</strong></section>
