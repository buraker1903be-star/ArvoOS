...
export default function PanelHome() {
  ...
  const dynamicStats: Partial<Record<ModuleKey, { label: string; value: string; change: string }[]>> = {
    ...
    finance: [{ label: "Tahsil edilen", value: currency(collected), change: `${paidDeals.length} ödeme` }, { label: "Bekleyen ödeme", value: currency(pending), change: "Tahsilat kuyruğu" }, { label: "Ortalama satış", value: currency(paidDeals.length ? collected / paidDeals.length : 0), change: "Tahsil edilen" }, { label: "Finans hareketi", value: String(paidDeals.length), change: "Otomatik oluştu" }],
  ...
  return (
    <div className={`panel-shell ${menuOpen ? "menu-open" : ""}`}>
      ...
      <main className="panel-main">
        ...
        {activeModule === "finance" && <section className="data-card"><div className="card-head"><div><small>TAHSİLAT HAREKETLERİ</small><h2>Ödemeden otomatik oluşan kayıtlar</h2></div><span className="live-note">{paidDeals.length} hareket</span></div><div className="data-table"><div className="data-heading"><span>Müşteri</span><span>Satıcı</span><span>Tarih</span><span>Durum</span><span>Tutar</span></div>{paidDeals.map((deal) => <div className="data-row" key={deal.id}><b>{deal.customer}</b><span>{deal.owner}</span><span>{deal.updated}</span><em>Tahsil edildi</em><strong>{currency(deal.amount)}</strong></div>)}</div></section>}
