...
export function MessagesDrawer({organizationId,userId,people,initialChannels}:{organizationId:string;userId:string;people:Person[];initialChannels:Channel[]}){
  ...
  useEffect(()=>{const handler=()=>{setLoading(true);setError("");setOpen(true)};window.addEventListener("arvo:open-messages",handler);return()=>window.removeEventListener("arvo:open-messages",handler);},[]);
  ...
  useEffect(()=>{if(!open||!activeId)return;let cancelled=false;supabase.from("internal_messages").select("id,channel_id,sender_id,body,created_at").eq("organization_id",organizationId).eq("channel_id",activeId).order("created_at").limit(150).then(({data,error})=>{if(cancelled)return;if(error)setError("Mesajlar yüklenemedi.");else setMessages((data??[]) as Message[]);setLoading(false)});return()=>{cancelled=true};},[open,activeId,organizationId,supabase]);
  useEffect(()=>{const subscription=supabase.channel(`team-chat-${organizationId}`)
    .on("postgres_changes",{event:"INSERT",schema:"public",table:"internal_messages",filter:`organization_id=eq.${organizationId}`},(payload)=>{const message=payload.new as Message;if(message.channel_id===activeId)setMessages((current)=>current.some((item)=>item.id===message.id)?current:[...current,message]);})
  ...
  return <>
    <button className="panel-quick-action" type="button" onClick={()=>{setLoading(true);setError("");setOpen(true)}} aria-label="Mesajları aç" aria-expanded={open} aria-controls="messages-drawer"><span className="panel-quick-icon" aria-hidden="true">◇</span><b>Mesajlar</b></button>
    <button className="messages-drawer-backdrop" type="button" aria-label="Mesajları kapat" onClick={()=>setOpen(false)} tabIndex={open?0:-1}/>
    <aside id="messages-drawer" className="messages-drawer" aria-hidden={!open}>
