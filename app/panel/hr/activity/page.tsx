...
export default async function PersonnelActivityPage(){
 ...
 const pathByUser=new Map<string,string>();for(const session of sessions){if(session.current_path&&!session.logout_at&&!pathByUser.has(session.user_id))pathByUser.set(session.user_id,session.current_path);}
