...
export default async function ReportingPage({ searchParams }: { searchParams: Promise<{ aralik?: string; baslangic?: string; bitis?: string }> }) {
  ...
  return <>
    <div className="panel-pagehead report-pagehead"><div><small className="panel-kicker">YÖNETİCİ RAPORLARI</small><h1>Satış ve Gerçek Kârlılık</h1><p>İmzalı sözleşme, tahsilat ve iş maliyeti kayıtlarından oluşan finansal yönetim özeti.</p></div><div className="panel-page-actions"><span className="status-pill">{rangeLabel}</span><PrintReportButton /></div></div>
    ...
    <section className="report-kpi-grid">
      ...
      <article className="report-kpi accent"><small>DÖNÜŞÜM</small><strong>{percent(conversion)}</strong><p>Görüşmeden satışa</p></article><article className="report-kpi warning"><small>İŞ MALİYETİ</small><strong>{money(contractedCost)}</strong><p>Bu dönemin imzalı işleri</p></article><article className={`report-kpi ${grossProfit>=0?"success":"danger"}`}><small>BRÜT İŞ KÂRI</small><strong>{money(grossProfit)}</strong><p>{percent(grossMargin)} sözleşme marjı</p></article><article className="report-kpi"><small>TAHSİLAT</small><strong>{money(collections)}</strong><p>Kasaya giren müşteri ödemesi</p></article><article className={`report-kpi ${realizedProfit>=0?"success":"danger"}`}><small>NET NAKİT HAREKETİ</small><strong>{money(realizedProfit)}</strong><p>{percent(realizedMargin)} tahsilat sonrası fark</p></article>
    </section>
    <div className="report-main-grid">
      ...
      <section className="panel-card report-card report-analysis"><header><div><small className="panel-kicker">YÖNETİCİ ANALİZİ</small><h2>Bu dönem ne söylüyor?</h2></div></header><div className="report-insights"><article><i>1</i><div><b>En zayıf geçiş</b><p>{weakestStep ? `${weakestStep.from} → ${weakestStep.to} dönüşümü ${percent(weakestStep.rate)}.` : "Yeterli veri yok."}</p></div></article><article><i>2</i><div><b>Sözleşme kârlılığı</b><p>{sales?`${money(salesValue)} imzalı işin maliyeti ${money(contractedCost)}; brüt marj ${percent(grossMargin)}.`:"Bu dönemde imzalanan sözleşme yok."}</p></div></article><article><i>3</i><div><b>Net nakit hareketi</b><p>{realizedProfit>=0?`${money(collections)} tahsilattan ${money(paidJobCost)} ödenmiş iş maliyeti ve ${money(operatingExpense)} genel gider sonrası ${money(realizedProfit)} kaldı.`:`Bu dönemde ödemeler tahsilatı ${money(Math.abs(realizedProfit))} aşıyor; bu değer muhasebesel zarar değil, dönemsel nakit farkıdır.`}</p></div></article><article><i>4</i><div><b>Referans gücü</b><p>{referralLeads.length ? `${referralLeads.length} referans lead’in ${referralSales} tanesi satışa döndü.` : "Bu dönemde referans kaynaklı lead kaydı yok."}</p></div></article></div></section>
    </div>
