// ArvoOS alt sayfaları, hizmet sayfaları ve Hakkımızda için ortak açık şablon.
  ...
  return (
    <>
      ...
      <div className="scenes">
        <section className="section" aria-labelledby="cards-title">
          <div className="wrap">
            <SectionHead eyebrow={c.cards.eyebrow} title={c.cards.title} lead={c.cards.lead} split={Boolean(c.cards.lead)} id="cards-title" />
            {hasBento(c.cards.items) ? <Bento items={c.cards.items} locale={locale} numbered={c.cards.numbered} /> : (
                ...
                {c.cards.items.map((card, i) => {
                  const body = (
                    <>
                      ...
                      {card.href ? <span className="link">{locale === "tr" ? "Daha fazla bilgi" : "Learn more"} ›</span> : null}
                    </>
                  );
        ...
        {c.steps ? (
          <section className="section" aria-labelledby="steps-title">
            <div className="wrap">
              ...
              <ol className="proc">{c.steps.items.map((s, i) => <li key={s.title} data-reveal style={{ ["--i" as string]: i % 3 }}><h3>{s.title}</h3><p>{s.text}</p></li>)}</ol>
