      ...
      <div>
          ...
          <section className="section tint" aria-labelledby="flow-title">
            <div className="wrap story">
              <div className="story-pin">
                ...
                <h2 id="flow-title" className="h2" style={{ marginTop: 14 }}>{c.flow.title}</h2>
                {c.flow.lead ? <p className="lead">{c.flow.lead}</p> : null}
                <div className="rail" aria-hidden="true"><i /></div>
                {c.flow.cta ? <Actions items={[c.flow.cta]} className="after-grid" /> : null}
              </div>
              <ol className="steps">
                {c.flow.steps.map((s, i) => <li key={s.title} className="step"><span className="step-n">{String(i + 1).padStart(2, "0")}</span><div><h3>{s.title}</h3><p>{s.text}</p></div></li>)}
              </ol>
            </div>
          </section>
        ) : null}
        ...
        <section className="section" aria-labelledby="aud-title">
          <div className="wrap">
            ...
            <div className="aud">
              {c.audience.items.map((a) => a.href
                ? <Link key={a.title} href={a.href} data-reveal><h3>{a.title}</h3><p>{a.text}</p><span className="link">{c.audience.more} <span className="arrow" aria-hidden="true">→</span></span></Link>
                : <div key={a.title} data-reveal><h3>{a.title}</h3><p>{a.text}</p></div>)}
            </div>
          </div>
        </section>
