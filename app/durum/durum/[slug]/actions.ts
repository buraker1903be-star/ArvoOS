...
export async function lookupStatus(
  ...
  if (code.length < 6) {
    return { error: "Lütfen size gönderilen takip kodunu eksiksiz girin.", results: null };
  }

  const supabase = await createClient();
  const { data, error } = await supabase.rpc("lookup_contract_by_tracking_code", {
    p_org_slug: orgSlug,
  ...
  if (!data || data.length === 0) {
    return { error: "Girdiğiniz takip koduyla eşleşen bir sözleşme bulunamadı.", results: null };
  }
