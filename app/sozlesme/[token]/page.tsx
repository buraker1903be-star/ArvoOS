...
export default async function PublicContractPage({ params, searchParams }: { params: Promise<{ token: string }>; searchParams: Promise<{ signed?: string; workflow?: string; created?: string }> }) {
  ...
  return <main className="contract-page" style={{ "--brand": brandColor } as CSSProperties}>
    ...
    <article className="document">
      <header className="header"><div>{row.organization_logo_url ? <img className="logo" src={row.organization_logo_url} alt={`${row.organization_name} logosu`} /> : <div className="logo-fallback">{row.organization_name}</div>}</div><div className="meta"><strong>{row.contract_no}</strong><span>Düzenleme: {date(row.created_at)}</span><br/><span>Şablon: {template.name}</span><br/><span>Sürüm: {template.version}</span></div></header>
