...
export function ContractAddenda({
  ...
  return <section className="doc-sec" id="ek-protokoller">
    ...
    {visible.map((addendum) => {
      ...
      return <article key={addendum.id} id={`ek-protokol-${addendum.addendum_no}`} className={`doc-addendum is-${addendum.status}`}>
        <header className="doc-addendum-head">
          <div>
            <div className="doc-kicker">Ek Protokol {addendum.addendum_no} · {contractNo}</div>
            <h3>İş Planı ve Ödeme Takvimi</h3>
            <small>Düzenleme: {formatDateTime(addendum.created_at)}</small>
          </div>
          <span className={addendum.status === "accepted" ? "doc-pill doc-pill-ok" : addendum.status === "rejected" ? "doc-pill doc-pill-bad" : "doc-pill"}>{ADDENDUM_STATUS_LABELS[addendum.status]}</span>
        </header>
        <p>
        ...
        {addendum.payment_dates.length ? <>
          ...
          <div className="doc-table-wrap"><table className="doc-table doc-table-compact">
            <thead><tr>
              <th style={{ width: "8%" }}>No</th>
              <th>Ödeme</th>
              <th className="doc-num" style={{ width: "22%" }}>Tutar</th>
            ...
            <tbody>{addendum.payment_dates.map((item) => {
              ...
              return <tr key={item.sequence}>
                ...
                <td className="doc-num">{row ? formatMoney(row.amount, currency) : "—"}</td>
                <td><strong>{formatDate(item.due_date)}</strong></td>
              </tr>;
            })}</tbody>
          </table></div>
        </> : null}
        {addendum.note ? <p className="doc-addendum-note"><b>Hizmet Sağlayıcı notu:</b> {addendum.note}</p> : null}
        {decided ? <div className={addendum.status === "accepted" ? "doc-decision" : "doc-decision is-rejected"}>
          <div className="doc-decision-badge"><i>{addendum.status === "accepted" ? "✓" : "✕"}</i>{addendum.status === "accepted" ? "Müşteri tarafından elektronik olarak onaylandı" : "Müşteri değişiklik istedi; bu ek protokol yürürlüğe girmedi"}</div>
          {addendum.status === "rejected" && addendum.response_note ? <p className="doc-addendum-note">“{addendum.response_note}”</p> : null}
        ...
        {addendum.status === "sent"
          ? (!print && renderActions ? renderActions(addendum) : <div className="doc-esign doc-esign-pending">Müşteri onayı bekleniyor</div>)
          : null}
      </article>;
    })}
  </section>;
