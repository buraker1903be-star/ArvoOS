...
//
// KALIP 1 — sınırlı sorgudan toplam.
...
/** `{ data: x, error }` içinden x'i çıkarır. */
const dataAdi = (kalip) => kalip.match(/\bdata\s*:\s*([A-Za-z_$][\w$]*)/)?.[1] ?? null;
  ...
  for (const dosya of dosyalar(tam)) {
    ...
    */
    ...
    const kacisVar = (no) => {
      ...
      for (let i = no - 2; i >= 0 && yorumMu(satirlar[i] ?? ""); i -= 1) {
...
console.log(`✓ Rakam denetimi: sınırlı sorgudan toplam yok, ${TUZAKLAR.length} tuzak tablo/sütun gözetiliyor`);
