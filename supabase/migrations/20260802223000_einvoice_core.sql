...
create table if not exists public.sales_invoice_lines (
  ...
  created_at timestamptz not null default now(),
  constraint sales_invoice_lines_invoice_org_fk foreign key (sales_invoice_id, organization_id)
    references public.sales_invoices(id, organization_id) on delete cascade
);
...
grant select, insert, update, delete on public.invoice_series, public.sales_invoices, public.sales_invoice_lines to authenticated;
...
create or replace function public.next_invoice_number(p_organization_id uuid,p_document_type text,p_prefix text)
returns text language plpgsql security definer set search_path='' as $$
declare y integer:=extract(year from current_date)::integer; n bigint;
begin
  if not exists (
    select 1 from public.organization_memberships m
    where m.organization_id=p_organization_id and m.user_id=auth.uid() and m.is_active and m.role in ('owner','admin')
  ) then raise exception 'Not authorized'; end if;
  if p_document_type not in ('e_invoice','e_archive') or upper(p_prefix) !~ '^[A-Z0-9]{3}$' then
...
revoke all on function public.next_invoice_number(uuid,text,text) from public,anon;
grant execute on function public.next_invoice_number(uuid,text,text) to authenticated;
