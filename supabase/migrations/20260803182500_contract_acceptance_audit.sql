...
drop function if exists public.sign_crm_contract(text, text);

create function public.sign_crm_contract(
  public_token text,
  signer_name text,
...
begin
  ...
  if con.id is null then raise exception 'invalid_token'; end if;
  if con.status = 'signed' then return query select 'signed', con.workflow_id; return; end if;
  if length(trim(coalesce(signer_name, ''))) < 2 then raise exception 'invalid_signer'; end if;

  select * into opp from public.crm_opportunities where id = con.opportunity_id;
  select slug into org_slug from public.organizations where id = con.organization_id;
  template_key := case
  ...
  if party is null then
    ...
    values(con.organization_id, 'customer', opp.customer_name, opp.contact_email, opp.contact_phone, true, con.created_by)
  ...
  values(con.organization_id, con.title, opp.customer_name, con.scope, 'planned', 'normal', coalesce(con.start_date, current_date), con.due_date, con.created_by)
  returning id into new_wf;

  insert into public.operation_steps(organization_id, workflow_id, title, sort_order) values
  ...
  values(con.organization_id, con.id, party, con.amount, con.currency, 'active', con.created_by)
  returning id into plan;

  if jsonb_typeof(proposal_schedule) = 'array' and jsonb_array_length(proposal_schedule) > 0 then
  ...
  if installment_count = 0 then
    insert into public.payment_installments(organization_id, payment_plan_id, installment_no, due_date, amount, status)
    values(con.organization_id, plan, 1, due_on, con.amount, 'pending');
  end if;
  ...
  values(con.organization_id, party, 'debit', 'crm_contract', con.amount, con.currency, con.title, con.contract_no, current_date, due_on, con.created_by);

  insert into public.finance_transactions(organization_id, transaction_type, status, title, counterparty, category, amount, currency, due_date, notes, created_by)
  values(con.organization_id, 'income', 'planned', con.title, opp.customer_name, 'Sözleşme Tahsilatı', con.amount, con.currency, due_on, 'Sözleşme ' || con.contract_no || ' üzerinden otomatik oluşturuldu.', con.created_by);

  insert into public.billing_invoices(organization_id, provider, status, currency, subtotal, tax, total, due_at)
  values(con.organization_id, 'manual', 'draft', con.currency, con.amount, 0, con.amount, due_on::timestamptz)
