...
create policy "members_read_available_channels" on public.message_channels for select to authenticated using (
  exists(
    ...
    where m.organization_id=message_channels.organization_id
  ...
  and (
    ...
    or exists(
      ...
      where cm.channel_id=message_channels.id
        and cm.organization_id=message_channels.organization_id
...
create policy "self_or_admin_read_channel_members" on public.message_channel_members for select to authenticated using (
  ...
  or exists(
    select 1 from public.organization_memberships m
    where m.organization_id=message_channel_members.organization_id
      and m.user_id=(select auth.uid())
      and m.is_active
      and m.role in ('owner','admin')
  )
...
create policy "admins_manage_channel_members" on public.message_channel_members for all to authenticated using (
  ...
  exists(
    select 1 from public.organization_memberships admin_membership
    where admin_membership.organization_id=message_channel_members.organization_id
      and admin_membership.user_id=(select auth.uid())
      and admin_membership.is_active
      and admin_membership.role in ('owner','admin')
  )
  and exists(
    select 1 from public.organization_memberships target_membership
    where target_membership.organization_id=message_channel_members.organization_id
      and target_membership.user_id=message_channel_members.user_id
...
create policy "members_read_channel_messages" on public.internal_messages for select to authenticated using (
  exists(
    select 1 from public.organization_memberships m
    where m.organization_id=internal_messages.organization_id
      and m.user_id=(select auth.uid())
      and m.is_active
  )
  and exists(
    ...
    where c.id=internal_messages.channel_id
      and c.organization_id=internal_messages.organization_id
      and (
        not c.is_private
        or exists(
          select 1 from public.message_channel_members cm
