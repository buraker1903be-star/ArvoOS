-- sign_crm_contract already resolves/creates the account_parties row (`party`)
-- but never links it on the finance_transactions row it inserts, so a payment
-- made against that transaction had no way to find and settle the matching
-- cari balance. This links every future contract's income record to its party.
...
begin
  ...
  insert into public.finance_transactions(
    ...
    counterparty, party_id, category, amount, currency,
    due_date, notes, created_by
  ) values (
    con.organization_id, 'income', 'planned', con.title,
    opp.customer_name, party, 'Sözleşme Tahsilatı', con.amount,
    con.currency, due_on,
